...
- Optional color overlay by a numeric seed column (e.g., prob).
- Graceful fallback if Cartopy is not available.
"""
...
# Try cartopy, but don't die if missing
try:
...
def pick_col_fuzzy(df, candidates):
    ...
    # build map of canonicalized existing columns -> original
    lc_map = {}
    for c in df.columns:
        k = canon(c)
        # keep first seen to avoid losing columns on collisions
        if k not in lc_map:
            lc_map[k] = c
...
def sanitize_name(nm: str) -> str:
    return re.sub(r'[^A-Za-z0-9_\-]+', '_', str(nm)).strip('_') or "storm"
...
def load_ibtracs(path, normalize_lon_mode, start=None, end=None, time_offset_h=0.0, area=None):
    ...
    lon_col  = pick_col_fuzzy(df, [canon(c) for c in IB_LON])
    vmax_col = pick_col_fuzzy(df, [canon(c) for c in IB_VMAX])
...
def load_seeds(path, normalize_lon_mode, override_lat=None, override_lon=None, override_time=None):
    ...
    # Keep any extra numeric columns for overlay (e.g., prob)
    extras = [c for c in m.columns if c not in {lat_col, lon_col, time_col, (name_col or "")}]
    for c in extras:
        if c not in seeds.columns:
            seeds[c] = pd.to_numeric(m[c], errors="ignore")
...
def merge_track_fragments(df_storm, gap_hours=48, max_km=500):
    ...
    # if times are NaT (shouldn't be), guard
    valid = srt["time"].notna()
    srt = srt.loc[valid].reset_index(drop=True)
    if srt.empty:
        return srt
...
def _auto_extent(track_df, seeds_df):
    ...
    return [lon_all.min()-lon_pad, lon_all.max()+lon_pad,
            lat_all.min()-lat_pad, lat_all.max()+lat_pad]

def plot_one(storm_name, track_df, seeds_df, out_png, dpi=160, title=None, overlay_col=None):
    extent = _auto_extent(track_df, seeds_df)
    if extent is None:
        return False

    # Fallback path: no Cartopy
    if not CARTOPY_OK:
        plt.figure(figsize=(10,6))
        # tracks
        td = track_df.sort_values("time")
        plt.plot(td["lon"], td["lat"], color="#1f77b4", linewidth=2.0, alpha=0.9, label="Track")
        # endpoints
        td_valid = td.dropna(subset=["time","lat","lon"])
        if not td_valid.empty:
            plt.scatter([td_valid.iloc[0]["lon"]],[td_valid.iloc[0]["lat"]],
                        s=60, marker="^", color="green", edgecolor="k", label="Storm start")
            plt.scatter([td_valid.iloc[-1]["lon"]],[td_valid.iloc[-1]["lat"]],
                        s=60, marker="v", color="red", edgecolor="k", label="Storm end")
        # seeds
        if overlay_col and overlay_col in seeds_df.columns:
            vals = pd.to_numeric(seeds_df[overlay_col], errors="coerce")
            sc = plt.scatter(seeds_df["seed_lon"], seeds_df["seed_lat"], s=24, c=vals, cmap="viridis",
                             alpha=0.85, edgecolor="k", linewidths=0.3, label="Seed starts")
            cb = plt.colorbar(sc)
            cb.set_label(overlay_col)
        else:
            plt.scatter(seeds_df["seed_lon"], seeds_df["seed_lat"], s=24,
                        color="#f5a623", edgecolor="k", linewidths=0.3, alpha=0.85, label="Seed starts")
        plt.xlim(extent[0], extent[1]); plt.ylim(extent[2], extent[3])
        plt.xlabel("Longitude"); plt.ylabel("Latitude")
        plt.title(title or f"Seed–Track Overlay — {storm_name}")
    ...
    try:
        ...
        # Older cartopy versions sometimes need different args
        ax.gridlines(linewidth=0.3, alpha=0.6, linestyle='--')
    ...
    if not seeds_df.empty:
        if overlay_col and overlay_col in seeds_df.columns:
            vals = pd.to_numeric(seeds_df[overlay_col], errors="coerce")
            sc = ax.scatter(seeds_df["seed_lon"], seeds_df["seed_lat"], s=24, c=vals, cmap="viridis",
    ...
    ax.set_title(title or f"Seed–Track Overlay — {storm_name}", fontsize=15)
...
def main():
    ...
    # explicit seed column overrides
    ...
    # optional color overlay for seeds
    ...
    for nm in storms:
        ...
        # if seeds have names, filter; else plot seeds in extent
        if "name" in seeds.columns and seeds["name"].notna().any():
            sdf = seeds.loc[seeds["name"].astype(str).str.upper() == nm].copy()
        else:
            sdf = seeds.copy()

        out_png = out_dir / f"{nm_clean}.png"
