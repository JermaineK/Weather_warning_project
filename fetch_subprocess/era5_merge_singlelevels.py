...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Merge ERA5 single-level NetCDFs per month into union files.")
    ap.add_argument("--in-glob", required=True, help='Glob of input .nc files, e.g. "data_era5/extracted/**/era5_*.nc"')
    ap.add_argument("--out-dir", required=True, help="Where to write era5_single_YYYYMM_<suffix>.nc")
    ap.add_argument("--suffix", default="oper", help="Suffix for output filenames (default: oper)")
...
def standardize_coords(ds: xr.Dataset) -> xr.Dataset:
    ...
    # try to decode CF-times if necessary
    try:
        ds = xr.decode_cf(ds)
    except Exception:
        pass
...
def open_one_nc(path: Path) -> xr.Dataset:
    # Try multiple engines for maximum tolerance
    last_err = None
    for eng in (None, "netcdf4", "h5netcdf"):
        try:
            ...
            last_err = e
            continue
    raise RuntimeError(f"Failed to open/standardize {path} : {last_err}")
...
def merge_month(files: List[Path], out_path: Path, quiet: bool = False) -> None:
    ...
    for i, p in enumerate(sorted(files)):
        ...
        # some inputs (e.g., per-variable month) already aligned; ensure index types comparable
        ...
        if merged is None:
            ...
            # Use merge to align by coords. Override attrs inconsistencies.
...
def main():
    ...
    for yyyymm, files in sorted(buckets.items()):
        year = yyyymm[:4]; month = yyyymm[4:]
        out_path = Path(args.out_dir) / year / month / f"era5_single_{yyyymm}_{args.suffix}.nc"
