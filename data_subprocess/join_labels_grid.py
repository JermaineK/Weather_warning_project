...
def parse_args():
    ...
    # Labeling knobs
    # Accept both underscore and hyphenated variants so manager/CLI drift is harmless.
    ap.add_argument(
        "--storm_radius_deg", "--storm-radius-deg",
        dest="storm_radius_deg",
        type=float,
        default=1.0,
    )
    ap.add_argument(
        "--storm_time_h", "--storm-time-h",
        dest="storm_time_h",
        type=float,
        default=3.0,
    )
    ap.add_argument(
        "--near_radius_deg", "--near-radius-deg",
        dest="near_radius_deg",
        type=float,
        default=5.0,
    )
    ap.add_argument(
        "--near_time_h", "--near-time-h",
        dest="near_time_h",
        type=float,
        default=12.0,
    )
...
def main():
    ...
    if lab.empty:
        print(
            "[warn] labels file parsed but no valid rows after time/lat/lon normalization. "
            "All labels will be zeros/NaN. Check track subset, AOI, and lon mode.",
            file=sys.stderr,
        )
    else:
        print(
            f"[labels] rows={len(lab):,}  time={lab['time'].min()} -> {lab['time'].max()}  "
            f"lat={lab['lat'].min():.3f}..{lab['lat'].max():.3f}  "
            f"lon={lab['lon'].min():.3f}..{lab['lon'].max():.3f}"
        )
    ...
    try:
        ...
        if probe is not None and {"time", "storm_point", "storm_window"}.issubset(probe.columns):
            ...
            if "t_to_storm_min_h" in probe.columns:
                tt = pd.to_numeric(probe["t_to_storm_min_h"], errors="coerce")
                finite = tt[np.isfinite(tt)]
                finite_in = finite[(finite >= 0) & (finite <= max_future)]
                print(
                    f"[labels] t_to_storm_min_h: sample finite={len(finite):,} "
                    f"finite<=max_future={len(finite_in):,} "
                    f"min={finite.min() if len(finite) else 'nan'} "
                    f"max={finite.max() if len(finite) else 'nan'}"
                )
    except Exception:
        pass
