...
def to_parquet_path(p: Path) -> Path:
    # If the filename actually ends with .csv, swap it; otherwise just force .parquet
    return p.with_suffix(".parquet") if p.suffix else Path(str(p) + ".parquet")
...
def _run_lengths_per_point(df: pd.DataFrame, time_col: str) -> pd.DataFrame:
    dfx = df.copy()
    dfx["time_h"] = pd.to_datetime(dfx[time_col], utc=True).dt.tz_localize(None).dt.floor("h")
    out = []
    for (lat, lon), g in dfx.groupby(["lat","lon"], sort=False):
        ts = g["time_h"].dropna().sort_values().unique()
        if len(ts) == 0: continue
        runs, run_len = [], 1
        for i in range(1, len(ts)):
            if (ts[i] - ts[i-1]).total_seconds() == 3600:
                run_len += 1
            else:
...
def main():
    ...
    ap.add_argument("--out-prefix", default="", help="Optional filename prefix for outputs.")
    ...
    if bad and not args.quiet:
        print(f"[info] dropping {bad:,} rows with invalid time.")
    ...
    if flag_col is None:
        if prob_col is not None:
            df["__flag__"] = (pd.to_numeric(df[prob_col], errors="coerce").fillna(0) > float(args.prob_thr)).astype(int)
            flag_col = "__flag__"
        else:
            df["__flag__"] = 1
            flag_col = "__flag__"
    ...
    by_hour = df.groupby("time_h", sort=True, as_index=True)
    hourly_count = by_hour[flag_col].sum().rename("active_cells")
    hourly_count.index.name = "time_h"
    if prob_col:
    ...
    hot = (
        df.groupby(["lat","lon"], sort=False)[flag_col]
          .sum().reset_index().rename(columns={flag_col:"hits"})
          .sort_values("hits", ascending=False).reset_index(drop=True)
    )
    topk = hot.head(int(args.topk))

    runs = _run_lengths_per_point(
        df[[time_col,"time_h","lat","lon",flag_col]].loc[df[flag_col] == 1],
        time_col=time_col
    )
    if not runs.empty:
    ...
    if len(files) == 1:
        ...
        report_path = out_dir / f"{prefix}{stem}_analysis.txt"
    else:
        report_path = out_dir / f"{prefix}seeds_analysis.txt"

    hourly_path   = out_dir / f"{prefix}seeds_hourly.csv"
    day_path      = out_dir / f"{prefix}seeds_per_day.csv"
    hotspots_path = out_dir / f"{prefix}seeds_hotspots.csv"
    runs_path     = out_dir / f"{prefix}seeds_runs.csv"
    bins_path     = out_dir / f"{prefix}seeds_spatial_bins.csv"

    # Write CSVs (and optional Parquet)
    ...
    if args.save_parquet:
        hourly.reset_index().to_parquet(to_parquet_path(hourly_path), index=False)
        per_day.reset_index().to_parquet(to_parquet_path(day_path), index=False)
        topk.to_parquet(to_parquet_path(hotspots_path), index=False)
        runs_summary.to_parquet(to_parquet_path(runs_path), index=False)
        bins_df.to_parquet(to_parquet_path(bins_path), index=False)
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        if len(hourly) > 0:
            ...
            f.write(f"  mean={float(np.mean(a)):.1f}  median={float(np.median(a)):.1f}  "
                    ...
                    f"max={int(np.max(a))}\n")
            topH = hourly.reset_index().sort_values("active_cells", ascending=False).head(5)
            f.write("  Top hours:\n")
