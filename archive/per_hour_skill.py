#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
per_hour_skill.py — robust per-hour coverage & F1 for a given lead.

Reads labelled+alerts (CSV/CSV.GZ/Parquet), tolerates messy CSVs (fallback to python engine),
and writes tidy CSV: [issue_hour, coverage, f1, n].
"""

from __future__ import annotations
import argparse, os, sys
import numpy as np
import pandas as pd
from typing import Optional

CANDIDATE_TIME_COLS = ["time","time_h","datetime","valid_time","forecast_time"]

# ---------------- I/O ----------------

def read_any(path: str, usecols: Optional[list]=None) -> pd.DataFrame:
    p = str(path)
    if not os.path.exists(p):
        raise FileNotFoundError(p)
    low = p.lower()
    if low.endswith((".parquet",".pq")):
        return pd.read_parquet(p, columns=usecols)

    if os.path.getsize(p) == 0:
        raise RuntimeError(f"{p} is empty (0 bytes). Was the previous step interrupted?")

    attempts = [
        dict(engine="c", compression="infer"),
        dict(engine="python", compression="infer"),
        dict(engine="python", compression=None),
    ]
    for opts in attempts:
        try:
            return pd.read_csv(p, usecols=usecols, **opts, on_bad_lines="warn")
        except Exception:
            continue
    raise RuntimeError(f"Failed to read {p} with any fallback parser.")

def pick_time_col(df: pd.DataFrame, preferred="time") -> str:
    if preferred in df.columns:
        return preferred
    for c in CANDIDATE_TIME_COLS:
        if c in df.columns:
            return c
    raise ValueError(f"No time-like column among {CANDIDATE_TIME_COLS}")

def to_utc_naive(s: pd.Series) -> pd.Series:
    raw = s.astype(str).str.strip().str.replace("Z","",regex=False)
    t = pd.to_datetime(raw, utc=True, errors="coerce", infer_datetime_format=True)
    if t.notna().mean() > 0.5:
        return t.dt.tz_localize(None)
    # numeric fallback
    num = pd.to_numeric(raw, errors="coerce")
    if num.notna().any():
        mid = np.nanmedian(num)
        unit = "ms" if mid > 1e11 else "s"
        t = pd.to_datetime(num, unit=unit, utc=True, errors="coerce")
        return t.dt.tz_localize(None)
    return pd.to_datetime(raw, utc=True, errors="coerce").dt.tz_localize(None)


# ---------------- labels & metrics ----------------

def future_max_label_by_point(df: pd.DataFrame, target_col: str, hours: int) -> np.ndarray:
    """
    Compute max(target) in future [t, t+hours] window for each (lat,lon) time series.
    Uses time-based rolling for true temporal spacing.
    """
    out_parts = []
    df = df.sort_values(["lat","lon","time"], kind="mergesort").copy()
    for (_, _), g in df.groupby(["lat","lon"], sort=False):
        g = g.dropna(subset=["time"])
        if g.empty:
            continue
        y = pd.Series(pd.to_numeric(g[target_col], errors="coerce").fillna(0).astype(int).values,
                      index=pd.to_datetime(g["time"]))
        # reverse so rolling is forward-looking
        y_rev = y.iloc[::-1]
        futmax = y_rev.rolling(f"{hours}h", min_periods=1).max().iloc[::-1]
        out_parts.append(pd.Series(futmax.values, index=g.index))
    if not out_parts:
        return np.zeros(len(df), dtype=int)
    out = pd.concat(out_parts).sort_index()
    return out.reindex(df.index).fillna(0).astype(int).to_numpy()


def f1_from_flags(y_true: np.ndarray, flags: np.ndarray) -> float:
    tp = np.sum((flags==1) & (y_true==1))
    fp = np.sum((flags==1) & (y_true==0))
    fn = np.sum((flags==0) & (y_true==1))
    prec = tp / (tp + fp) if (tp+fp) else 0.0
    rec  = tp / (tp + fn) if (tp+fn) else 0.0
    return (2*prec*rec / (prec + rec)) if (prec+rec) else 0.0


# ---------------- main ----------------

def main():
    ap = argparse.ArgumentParser(description="Per-hour coverage & F1 for a given lead.")
    ap.add_argument("--labelled", required=True)
    ap.add_argument("--alerts", required=True)
    ap.add_argument("--lead-hours", type=int, required=True)
    ap.add_argument("--target", default="pregen")
    ap.add_argument("--flag-col", default="alert_final")
    ap.add_argument("--time-col", default="time")
    ap.add_argument("--out", default=None)
    ap.add_argument("--quiet", action="store_true")
    args = ap.parse_args()

    base = read_any(args.labelled).replace([np.inf,-np.inf], np.nan)
    alrt = read_any(args.alerts).replace([np.inf,-np.inf], np.nan)

    tcol_base = args.time_col if args.time_col in base.columns else pick_time_col(base)
    tcol_alrt = args.time_col if args.time_col in alrt.columns else pick_time_col(alrt)

    for c in [tcol_base,"lat","lon",args.target]:
        if c not in base.columns:
            sys.exit(f"[ERROR] labelled missing column {c}")
    fcol = args.flag_col
    if fcol not in alrt.columns:
        for alt in ["alert_throttled","alert_final","alert"]:
            if alt in alrt.columns:
                fcol = alt; break
    if fcol not in alrt.columns:
        sys.exit(f"[ERROR] alerts missing flag column: {args.flag_col}")

    base[tcol_base] = to_utc_naive(base[tcol_base])
    alrt[tcol_alrt] = to_utc_naive(alrt[tcol_alrt])
    base = base.dropna(subset=[tcol_base,"lat","lon",args.target]).reset_index(drop=True)
    alrt = alrt.dropna(subset=[tcol_alrt,"lat","lon",fcol]).reset_index(drop=True)

    for c in ["lat","lon"]:
        base[c] = pd.to_numeric(base[c], errors="coerce")
        alrt[c] = pd.to_numeric(alrt[c], errors="coerce")
    alrt[fcol] = pd.to_numeric(alrt[fcol], errors="coerce").fillna(0).astype(int)

    base["time_h"] = base[tcol_base].dt.floor("h")
    alrt["time_h"] = alrt[tcol_alrt].dt.floor("h")

    df = base[["time_h","time","lat","lon",args.target]].merge(
        alrt[["time_h","lat","lon",fcol]],
        on=["time_h","lat","lon"], how="left", validate="many_to_one"
    )
    df[fcol] = pd.to_numeric(df[fcol], errors="coerce").fillna(0).astype(int)

    if not args.quiet:
        print(f"[per-hour] computing future labels (+{args.lead_hours}h)...")
    y_future = future_max_label_by_point(df[["time","lat","lon",args.target]], args.target, int(args.lead_hours))
    df["y_future"] = y_future

    rows = []
    for t, g in df.groupby("time_h", sort=True):
        flags = g[fcol].to_numpy(int)
        y = g["y_future"].to_numpy(int)
        cov = float(flags.mean()) if len(flags) else 0.0
        f1 = f1_from_flags(y, flags)
        rows.append(dict(issue_hour=t, coverage=cov, f1=f1, n=len(g)))

    out = pd.DataFrame(rows).sort_values("issue_hour").reset_index(drop=True)
    out_path = args.out or os.path.join("results","per_hour",f"per_hour_skill_lead{int(args.lead_hours)}.csv.gz")
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    comp = "gzip" if out_path.endswith(".gz") else None
    out.to_csv(out_path, index=False, compression=comp, date_format="%Y-%m-%d %H:%M:%S")

    if not args.quiet:
        tmin, tmax = out["issue_hour"].min(), out["issue_hour"].max()
        print(f"[per-hour] wrote {out_path} rows={len(out)} time={tmin}->{tmax}")

if __name__ == "__main__":
    main()