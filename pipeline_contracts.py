...
STEP_CONTRACTS: Dict[Tuple[str, str], StepContract] = {
    ...
    ("data_stage", "lookup-panel"): StepContract(
        section="data_stage",
        mode="lookup-panel",
        inputs=[PathSpec(
            ("source", "infile", "in"),
            kind="file",
            required=True,
            required_columns=("row_id", "time", "lat", "lon"),
            produced_by=("data_stage.state-transitions",),
        )],
        outputs=[PathSpec(("out",), kind="file", required=True)],
        output_columns=["row_id", "time", "lat", "lon", "ilat", "ilon", "storm"],
    ...
    ("report", "object-matches"): StepContract(
        ...
        inputs=[
            ...
            PathSpec(
                ("cells", "cells_out", "cells-out"),
                kind="file",
                required=False,
                required_columns=("object_id", "row_id"),
                produced_by=("report.objects-by-hour",),
            ),
            PathSpec(
                ("flow_source", "objects_flow_source", "objects-flow-source", "storm_timeseries", "storm-timeseries"),
                kind="file",
                required=False,
                required_columns=("row_id", "u10", "v10"),
                produced_by=("features.integrate-thermo",),
            ),
