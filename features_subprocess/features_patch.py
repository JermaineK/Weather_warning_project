...
def _rolling_mean_std_lean(df: pd.DataFrame, col: str, win: int = 3) -> Tuple[pd.Series, pd.Series]:
    ...
    return (pd.Series(mean_out, index=df.index, dtype="float32"),
...
def _pressure_tendency_lean(df: pd.DataFrame):
    ...
    return (pd.Series(out1, index=df.index, dtype="float32"),
...
def _msl_grad_from_grid(df: pd.DataFrame) -> pd.Series:
    ...
    for _, idx in df.groupby("time", sort=False).indices.items():
        ...
        msl_map = dict(zip(key, pd.to_numeric(block["msl"], errors="coerce").to_numpy(dtype=float)))
        il = block["ilat"].to_numpy()
        jl = block["ilon"].to_numpy()
        m0 = pd.to_numeric(block["msl"], errors="coerce").to_numpy(dtype=np.float32)
        ...
        dmdy = np.where(np.isfinite(m_up) & np.isfinite(m_down), 0.5*(m_down - m_up),
                        np.where(np.isfinite(m_down), m_down - m0,
                                 np.where(np.isfinite(m_up), m0 - m_up, 0.0))).astype(np.float32)
        dmdx = np.where(np.isfinite(m_left) & np.isfinite(m_right), 0.5*(m_right - m_left),
                        np.where(np.isfinite(m_right), m_right - m0,
                                 np.where(np.isfinite(m_left), m0 - m_left, 0.0))).astype(np.float32)
...
def _deformation_shear_from_uv(df: pd.DataFrame) -> pd.Series:
    ...
    for _, idx in df.groupby("time", sort=False).indices.items():
        ...
        def _grid_from(colname):
            ...
            arr[rel_i, rel_j] = pd.to_numeric(df.loc[idx, colname], errors="coerce").to_numpy(dtype=np.float32, copy=False)
            return arr
...
def add_all_feature_enhancements(df_in: pd.DataFrame,
                                 prefer_shear: str | None = None,
                                 s3_window: int = 3,
                                 lite_derivatives: bool = False) -> pd.DataFrame:
    ...
    if shear_col == "__proxy__":
        sbase = df["shear_proxy"] if "shear_proxy" in df.columns else pd.Series(0.0, index=df.index, dtype="float32")
        sname = "shear_proxy"
    else:
    ...
    for _, idx in idx_map.items():
        ...
        S3_out[ii] = x.rolling(s3_window, min_periods=1).mean().fillna(0.0).to_numpy(dtype=np.float32, copy=False)
    ...
    # Compact a few known heavy columns (idempotent)
    for c in [
        "msl_d1h", "msl_d3h",
...
def parse_args():
    ap = argparse.ArgumentParser(description="Feature patcher: tendencies/rolling/deformation shear + S3 and simple ops.")
    ...
    ap.add_argument("--prefer-shear", default=None,
    ...
    ap.add_argument("--ops", default=None,
                    help='Optional JSON with light ops: {"rename":{...},"drop":[...],"select":[...],"add_const":{...}}')
    ap.add_argument("--lite-derivatives", action="store_true",
...
def _read_table(path: str) -> pd.DataFrame:
    p = Path(path); low = p.suffix.lower()
